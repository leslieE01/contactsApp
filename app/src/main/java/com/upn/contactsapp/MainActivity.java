...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int PAGE_SIZE = 10;  // Número de elementos por página
    private int currentPage = 1;              // Página inicial
    private boolean isLoading = false;        // Estado de carga
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setUpRecyclerView();
        loadContacts(currentPage); // Cargar la primera página
        ...
        service.getAll(page, PAGE_SIZE).enqueue(new Callback<List<Contact>>() {
            ...
            public void onResponse(Call<List<Contact>> call, Response<List<Contact>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    // Log para verificar cuántos contactos se cargaron
                    Log.d("MAIN_APP", "Se cargaron " + newContacts.size() + " nuevos contactos");
        ...
        rvContacts.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                if (layoutManager != null && layoutManager.findLastVisibleItemPosition() == elementos.size() - 1) {
                    loadContacts(currentPage); // Cargar la siguiente página cuando se llega al final
                }
            }
        });
    }
